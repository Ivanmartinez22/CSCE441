...
Shape* Camera::recursiveBHV_Intersection(Ray ray, BVH* Node, float t0, float t1){
	
	if(!Node->intersection(ray)) return nullptr;
	if(Node->isLeaf){
		float t = t1;
	float t_iter = 0.0;
	Shape * min = nullptr;
		...
		for(int j = 0; j < Node->shapes.size(); j++){
			Triangle* triangle = static_cast<Triangle*>(Node->shapes[j]); 
			if(triangle->intersection(ray, t0, t1, t_iter)){
				//cout << "triangle pointer" << triangle << endl;
		...
		return min;
		 ...
		 Shape* hit1= recursiveBHV_Intersection(ray, Node->left, t0, t1);
		Shape*  hit2 = recursiveBHV_Intersection(ray, Node->right, t0, t1);
		if(hit1 == nullptr && hit2 == nullptr){
			return nullptr;
		}
		if(hit1 != nullptr && hit2 == nullptr){
			return hit1;
		}
		if(hit2 != nullptr && hit1 == nullptr){
			return hit2;
		}
		if(hit1 != nullptr  && hit2 != nullptr){
			float t1_ = hit1->t;
...
Shape* Camera::Hit(Ray ray, float t0, float t1, vector<Shape*> shapes, int &num_triangles){// could prove buggy but good start
	float t = t1;
	float t_iter = 0.0;
	Shape * min = nullptr;
	...
	for(int i = 0; i < shapes.size(); i++){
		if(shapes[i]->type == 1){
		...
		else if(shapes[i]->type == 3){//raw triangle
		...
		else if(shapes[i]->type == 4){//bounding box
			
			BVH* boundbox = static_cast<BVH*>(shapes[i]); 

			// if(boundbox->intersection(ray)){
			// 	//cout << "intersecting bounding" << endl;
				
			// 	for(int j = 0; j < boundbox->shapes.size(); j++){
			// 		Triangle* triangle = static_cast<Triangle*>(boundbox->shapes[j]); 
			// 		if(triangle->intersection(ray, t0, t1, t_iter)){
			// 			//cout << "intersecting triangle" << endl;
			// 			if(t_iter < t && (t_iter>= t0)){
			// 				min = boundbox->shapes[j];
			// 				t = t_iter;
			// 				min->t = t;
			// 			}
			// 		}
			// 	}
			// }
			// int num = recursiveBHV_Intersection(ray, boundbox);
			// if( num == -1){
			// 	min = nullptr;
			// }
			// else{
			...
			// 	cout << "test 2" << endl;
			// }
			min =  recursiveBHV_Intersection(ray, boundbox, t0, t1);
...
glm::vec3 Camera::ComputeRayColor(Ray ray, Shape * hit_shape, Scene* scene, int depth, float t0, float t1){
	...
	if(hit_shape->type == 1){
		...
		normal = glm::normalize(triangle->calculated_normal);
		// normal = (plane->normal);
	}
	...
	for(int i = 0; i < scene->lights.size();i++){ //some calculations may be wrong
		...
		// epsilon += 0.000001;
		if((Hit(Sray, epsilon, t_light, scene->shapes, i) == nullptr)){//nullptr means no hit no shadow
			intersection = 1.0; 
			// cout << "no shadow" << endl;
		}
		else{//shadow 
	...
	if((depth < 4)){
		Shape * shape_in_way = Hit(Rray, epsilon, t1, scene->shapes, i);
		Shape * shape_in_way2 = Hit(Rfray, epsilon, t1, scene->shapes, i);
...
void Camera::TakePicture(Scene *scene)
{
	...
	for(int i = 0; i < widthRes; i++){
		for(int j = 0; j < (heightRes); j++){
			...
			float t1 = 1000.0;
			int num_triangles = 0;
			Shape * hit_shape = Hit(camera_ray, t0, t1, scene->shapes, num_triangles);
			glm::vec3 color = glm::vec3(0.0);
			// cout << "hit pointer " << hit_shape << endl;
